...
def run(tag, env, parallel, runner, node_names, to_nodes, from_nodes):
    """Run the pipeline."""
    from arbitrage_using_kedro.run import main
...
def build_docs():
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=arbitrage_using_kedro"]
    )
...
def convert_notebook(all_flag, overwrite_flag, filepath):
    ...
    *Note*: Make sure your notebooks have unique names!
    ...
    """
    from arbitrage_using_kedro.run import ProjectContext
    ...
    kedro_package_name = "arbitrage_using_kedro"
